...
func testUserToManyPermissions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"users_permissions\" (\"user_id\", \"permission_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"users_permissions\" (\"user_id\", \"permission_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPermissions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpPermissions(t *testing.T) {
	var err error
...
func testUserToManySetOpPermissions(t *testing.T) {
	var err error
	...
	err = a.SetPermissions(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetPermissions(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Permissions[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Permissions[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpPermissions(t *testing.T) {
	var err error
	...
	err = a.AddPermissions(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemovePermissions(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Permissions) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Permissions[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Permissions[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `CreatedAt`: `timestamp with time zone`, `Name`: `text`, `Email`: `USER-DEFINED`, `PasswordHash`: `bytea`, `Activated`: `boolean`, `Version`: `integer`}
	_           = bytes.MinRead
)
