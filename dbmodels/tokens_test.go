...
func testTokensExists(t *testing.T) {
	...
	e, err := TokenExists(ctx, tx, o.Hash)
	if err != nil {
...
func testTokensFind(t *testing.T) {
	...
	tokenFound, err := FindToken(ctx, tx, o.Hash)
	if err != nil {
		t.Error(err)
	}
...
func testTokenToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Token)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testTokenToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	tokenDBTypes = map[string]string{`Hash`: `bytea`, `UserID`: `bigint`, `Expiry`: `timestamp with time zone`, `Scope`: `text`}
	_            = bytes.MinRead
)
