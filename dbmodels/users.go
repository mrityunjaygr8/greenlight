...
func (userL) LoadPermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"permissions\".\"id\", \"permissions\".\"code\", \"a\".\"user_id\""),
		qm.From("\"permissions\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Code, &localJoinCol)
		if err != nil {
...
func (o *User) SetPermissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Permission) error {
...
func (o *User) RemovePermissions(ctx context.Context, exec boil.ContextExecutor, related ...*Permission) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"users_permissions\" where \"user_id\" = $1 and \"permission_id\" in (%s)",
